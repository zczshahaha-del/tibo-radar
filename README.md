...
1. 解压 `Tibo-Radar-v0.5.4-macOS-arm64.zip`；
...
- 默认面板把 24/48 小时综合概率并列展示，并单列 24 小时内的“重置额度”
  和“重置卡”概率；AI 的三条主要依据按需展开；
...
第三方数据可能延迟或中断，因此面板顶部会显示最近一次成功刷新预测的具体
时间；旧结果会用橙色时间提示。

## 结果应该怎么读

- “重置额度”是直接恢复大范围用户额度；
- “重置卡”是发到账户、可在以后手动使用的 banked reset；
- “故障补发”只适用于明确故障时段内受影响的用户；
- 并列的 24/48 小时数字回答“重置额度或重置卡至少发生一种”的概率；
- 下方两行把未来 24 小时的“重置额度”和“重置卡”概率分开显示；
- 卡片只显示 AI 的中心估计，完整概率范围仍用于结果校验和通知判断；
- “详细依据”只展示最影响本次判断的三条支持、反面或历史背景证据；
