...
或千问负责理解原文，但不能自行生成百分比。历史模型提供动态概率范围，AI
单独显示近期有没有新信号；两者不会硬凑成一个未经验证的精确数字。
...
1. 解压 `Tibo-Radar-v0.4.1-macOS-arm64.zip`；
...
- “历史估计”来自至少 100 个过去时间窗的回放，并且必须比简单猜测更准；
- AI 新信号与历史范围分开显示，目前不会假装知道一条暗示应增加几个百分点；
- 将来回放明确包含 AI 语义信号并击败旧模型后，范围才会标成“已校准”。
- 历史数据缺失、过少、范围非法或来源过期时才会显示“暂无估计”。
