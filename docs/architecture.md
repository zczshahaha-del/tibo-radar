...
- `ProbabilityCalibration.swift`：检查样本量、概率范围和基线对比，并区分
  “历史估计”与“已校准”。
...
概率必须来自按时间顺序隐藏未来数据的历史回放。至少 100 个时间窗、范围
合法且击败简单猜测时，可以显示为“历史估计”；只有回放明确包含 AI 语义
信号并击败旧模型时，才标为“已校准”。AI 新信号单独展示，不做未经验证的
数值加减。任何数字都只显示范围，不显示假精确的单点概率。
