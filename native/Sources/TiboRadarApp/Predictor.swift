...
struct Predictor: Sendable {
    ...
    func predict(bundle: SourceBundle, now: Date = Date()) -> PredictionSnapshot {
        ...
        if let incident = recentIncident(
            ...
            evidence.append(Evidence(
                ...
                delta: delta,
                sourceURL: URL(string: incident.string("shortlink") ?? incident.string("source_url") ?? "")
            ))
        }
        ...
        let level: ProbabilityLevel = if combined24 >= 85 {
            .red
        ...
        let isStale = !bundle.cacheFallbacks.isEmpty
        ...
        let sourceNote = forecast.string("confidence_note")
            ?? "历史样本较少，概率只用于安排用量。"
    ...
    private func recentIncident(
        ...
        let values = (openAIStatus.array("incidents") ?? [])
        ...
        for value in values {
            ...
            let searchable = "\(incident.string("name") ?? "") \(incident.string("body") ?? "")".lowercased()
            let related = incident.bool("codex_related") == true
