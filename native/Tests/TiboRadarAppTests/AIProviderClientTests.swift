...
final class AIProviderClientTests: XCTestCase {
  ...
  func testQwenEnablesSearchAndParsesSharedSchema() async throws {
    ...
    XCTAssertTrue(requestText.contains("必须把 reply_context.parent、reply_context.quoted_post 与 Tibo 回复合起来理解"))
    XCTAssertTrue(requestText.contains("应上调普发重置卡概率，但不能因此上调全局重置概率"))
    XCTAssertTrue(requestText.contains("跨帖重复视为对普发重置卡的追加正面信号"))
    XCTAssertTrue(requestText.contains("published_at_beijing 和 published_at_pacific"))
    XCTAssertTrue(requestText.contains("3am on a tuesday"))
    XCTAssertTrue(requestText.contains("不得用“近期均为普通互动”把它合并掉"))
    XCTAssertTrue(requestText.contains("若 reply_context_status=missing"))
    XCTAssertTrue(requestText.contains("recent_tibo_context 是 Tibo 最新公开动态"))
